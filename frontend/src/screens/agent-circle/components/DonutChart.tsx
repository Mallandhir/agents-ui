...
export const DonutChart: React.FC<DonutChartProps> = ({
  ...
  const [labelPositions, setLabelPositions] = useState<
    Array<{ x: number; y: number; data: EntityData }>
  >([]);
  ...
  useEffect(() => {
    ...
    svg.selectAll(".segment-path").attr("fill", (d: any) => {
      ...
      const midAngle =
        ((d.startAngle + d.endAngle) / 2) * (180 / Math.PI) + currentRotation;
      const normalizedAngle = ((midAngle % 360) + 360) % 360;

      // Check if this segment is at the active position (left side)
      const isAtActivePosition =
        Math.abs(normalizedAngle - ACTIVE_POSITION) < 10;
  ...
  useEffect(() => {
    ...
    const startRotation = parseFloat(
      chartGroup.attr("data-prev-rotation") || "0"
    );
  ...
  const createGradients = (
    ...
    pieData.forEach((d) => {
      ...
      gradient
        .append("stop")
        .attr("offset", "0%")
        .attr("stop-color", "#E8B8F0");

      gradient
        .append("stop")
        .attr("offset", "100%")
        .attr("stop-color", "#D5A1E5");
    });
  };
  ...
  const updateLabelPositions = (
    ...
    const newPositions = pieData.map((d) => {
      const angle =
        ((d.startAngle + d.endAngle) / 2) * (180 / Math.PI) + rotation - 90;
      const radians = (angle * Math.PI) / 180;
      const x =
        Math.cos(radians) * (innerRadius + (radius - innerRadius) * 0.5) +
        width / 2;
      const y =
        Math.sin(radians) * (innerRadius + (radius - innerRadius) * 0.5) +
        height / 2;
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      ...
      {labelPositions.map((pos, index) => {
        const midAngle =
          ((getPieData()[index].startAngle + getPieData()[index].endAngle) /
            2) *
            (180 / Math.PI) +
          currentRotation;
        const normalizedAngle = ((midAngle % 360) + 360) % 360;
        ...
        return createPortal(
          ...
          >
            <ChartLabel
              entity={pos.data}
              isActive={isAtActivePosition && activeSegment === pos.data.id}
            />
          </div>,
